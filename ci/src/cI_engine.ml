...
let flush_states t snapshot =
  ...
  let record_target (_id, target) =
    ...
    let src_hash = Commit.hash (CI_target.head target.v) in
    CI_history.record history dk ~src_hash snapshot (String.Map.of_list jobs)
  in
...
let latest_state t target =
  t.dk >>= fun dk ->
  CI_history.lookup t.history dk (CI_target.of_v target.v) >|= CI_history.head
