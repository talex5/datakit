...
module Session = struct
  ...
  let connect = function
    | `Memory ->
      Lwt.return (`Memory (Session.Memory.create ()))
    | `Redis addr ->
      Redis_lwt.Client.connect addr >|= fun conn ->
      `Redis (Session_redis_lwt.of_connection conn)
end
...
let server ~auth ~web_config ~session_backend =
  ...
  in
  Session.connect session_backend >|= fun session_backend ->
