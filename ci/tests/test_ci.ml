...
let test_history conn =
  ...
  let src_hash = "123" in
  let head = Commit.v repo src_hash in
  ...
  CI_history.record master_h dk ~src_hash metadata_commit s1 >>= fun () ->
  (* Check s1 *)
  ...
  in
  CI_history.record master_h dk ~src_hash metadata_commit s2 >>= fun () ->
  (* Check s2 *)
